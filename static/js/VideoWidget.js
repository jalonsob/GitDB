...
function VideoWidget(id,panel,color,direction,content,width,height,x,y){
	if((width==undefined) && (height==undefined)){
		Widget.call(this,id,panel,color,12,9)
	...
	this.makeMenu= function(){
		...
		$("#currentCreation").append('<p>With : <input  width="5" placeholder="'+12+'" id="x" ></p> <p>Heigh: <input width="5" placeholder="'+9+'" id="y"></div></p>')
	...
	this.takeData= function(state){
		...
		if(draw){
			...
			this.content='<iframe src="'+direction+'" width="'+(this.gridsterWidth*28)+'" height="'+(this.gridsterheight*23)+'" frameborder="0" allowfullscreen></iframe>'
			this.square='<div id= "widget'+this.id+'" class="panel panel-primary widgetDrop" style="border-style: groove;border-color: black;border-width: 3px"><div id="header'+this.id+'" class="panel-heading" style="background-color:'+this.color+'">'+this.buttons+'</div><div id="'+this.id+'" class="panel-body">'+this.content+'</div></div>';
	...
	this.settings= function(){
		...
		$("#currentSettings").append('<p>With : <input  width="10" placeholder="'+12+'" id="x" > Heigh: <input width="10" placeholder="'+9+'" id="y"></div></p>')
